    ...
    <div className="space-y-8">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="text-sm text-gray-400">
           {t('home.dataUpdated')}: {marketData?.latest_date || new Date().toLocaleDateString()}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 relative overflow-hidden">
          <div className="relative z-10">
            <h3 className="text-lg font-semibold text-gray-700 flex items-center gap-2">
              <BarChart3 className="w-5 h-5 text-blue-500" /> {t('home.sseComposite')}
            </h3>
            
            {isLoading ? (
              <div className="h-20 flex items-center text-gray-400">{t('loading')}</div>
            ) : marketData ? (
              <div className="mt-4">
              ...
              <div className="text-red-400 mt-2">{t('home.unavailable')}</div>
            )}
          </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
          <h3 className="text-lg font-semibold text-gray-700 flex items-center gap-2">
            <Activity className="w-5 h-5 text-purple-500" /> {t('home.systemStatus')}
          </h3>
          <div className="mt-4 space-y-3">
            <div className="flex justify-between items-center text-sm">
              <span className="text-gray-500">{t('home.apiConnection')}</span>
              <span className="flex items-center gap-1 text-green-600 font-medium"><span className="w-2 h-2 rounded-full bg-green-500"></span> {t('home.active')}</span>
            </div>
            <div className="flex justify-between items-center text-sm">
              <span className="text-gray-500">{t('home.dataFeed')}</span>
             ...
             <div className="flex justify-between items-center text-sm">
              <span className="text-gray-500">{t('home.lastSync')}</span>
        ...
        <div className="bg-gradient-to-br from-blue-600 to-indigo-700 p-6 rounded-lg shadow-sm text-white">
          <h3 className="text-lg font-semibold flex items-center gap-2">
            <Zap className="w-5 h-5" /> {t('home.quickActions')}
          </h3>
          <div className="mt-4 space-y-2">
            <Link to="/technical" className="block w-full py-2 px-3 bg-white/10 hover:bg-white/20 rounded text-sm transition-colors flex justify-between items-center">
              {t('home.runDailyScan')} <ArrowRight className="w-4 h-4" />
            </Link>
            <Link to="/ai" className="block w-full py-2 px-3 bg-white/10 hover:bg-white/20 rounded text-sm transition-colors flex justify-between items-center">
              {t('home.askAI')} <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        <div className="p-4 border-b border-gray-200 bg-gray-50">
           <h3 className="font-semibold text-gray-800">{t('home.recentSignals')}</h3>
        </div>
        <div className="p-8 text-center text-gray-400 text-sm">
           {t('home.noSignals')} <Link to="/technical" className="text-blue-600 hover:underline">{t('home.technicalAnalysis')}</Link> {t('home.toStartScan')}
        </div>
      </div>
    </div>
  );
